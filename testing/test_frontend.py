...
def fake_api_data():
    return [
        {"id": 1,"airline": "TestAir", "flight_number": "TA123", "departure_city": "London","departure_time": "Morning",
                                "stops": 0,
                                ...
                                "days_left": 0,
                                "price": 42},
        {"id": 2,"airline": "MockJet", "flight_number": "MJ456", "departure_city": "Paris", "departure_time": "evening",
                                "stops": 1,
                                "arrival_time": "lunch",
                                "arrival_city": "LA",
                                "travel_class": "economy+",
...
def fake_pandas_data(fake_api_data):
    return pd.DataFrame({"id": 1, "airline": "TestAir", "flight_number": "TA123", "departure_city": "London","departure_time": "Morning",
                                "stops": 0,
                                "arrival_time": "afternoon",
                                "arrival_city": "Rome",
                                "travel_class": "economy",
...
def test_get_column_types(data_processing, mock_requests_get, fake_pandas_data):
    assert data_processing.get_column_types(fake_pandas_data) == {"id": int, "airline": str, "flight_number": str, "departure_city": str,"departure_time": str,
                                "stops": int,
                                "arrival_time": str,
                                "arrival_city": str,
                                "travel_class": str,
...
def test_check_data(data_processing, mock_requests_get, fake_pandas_data):
    mock_response = {"id": int, "airline": str, "flight_number": str, "departure_city": str,"departure_time": str,
                                "stops": int,
                                "arrival_time": str,
                                "arrival_city": str,
                                "travel_class": str,
                                ...
                                "days_left": int,
                                "price": int}
    weird_data = pd.DataFrame({"id": 1, "airline": "TestAir", "flight_number": "TA123", "departure_city": "London","departure_time": 4,
                                "stops": 0,
                                "arrival_time": "afternoon",
                                "arrival_city": "Rome",
                                "travel_class": "economy",
                                ...
                                "days_left": 0,
                                "price": 42},
        {"id": 2, "airline": "MockJet", "flight_number": "MJ456", "departure_city": "Paris", "departure_time": 5,
                                "stops": 1,
                                "arrival_time": "lunch",
                                "arrival_city": "LA",
                                "travel_class": "economy+",
    ...
    with patch("frontend.Frontend.Data_processing.get_column_types", return_value=mock_response):
        assert data_processing.check_data(fake_pandas_data) == "Data is valid"
        assert data_processing.check_data(fake_pandas_data.drop(columns=["airline"])) == "Missing column: airline"
        assert data_processing.check_data(weird_data) == "Incorrect data type in column: departure_time. Expected str."
# 4
@pytest.mark.unit
def test_convert_to_JSON(data_processing, mock_requests_get, fake_pandas_data, fake_api_data):
    assert data_processing.convert_to_JSON(fake_pandas_data) == fake_pandas_data.to_dict(orient="records")
# 5
@pytest.mark.unit
def test_post_data(data_processing, mock_requests_get, fake_pandas_data):
...
def test_geo_data_returns_coordinates(data_processing):
    ...
    with patch("frontend.Frontend.Data_processing.get_geolocator", return_value=mock_geolocator):
...
# 10
@pytest.mark.integration
def test_geo_data_intdegration(data_processing, mock_requests_get, fake_pandas_data):
...
def test_search_page_integration(data_processing, mock_requests_get, fake_pandas_data):
    app = web_app()
